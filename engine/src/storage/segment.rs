...
///
/// The trailing CRC (checked in [`MmapSegment::open`]) proves the bytes are intact,
/// NOT that a length/offset read *out of* those bytes is structurally valid. A
/// CRC-consistent but malformed segment — a `count` that overruns the mmap, from a
/// writer bug, a torn write that happens to re-pass CRC, or tampering — would
/// otherwise let the `from_raw_parts` casts below construct an out-of-bounds typed
