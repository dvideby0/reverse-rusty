...
use crate::cli::Cli;
use crate::handlers::{
    alias_import_method_not_allowed, alias_read_method_not_allowed, cluster_backup,
    ...
    ALIAS_IMPORT_BODY_LIMIT, ALIAS_READ_BODY_LIMIT, BACKUP_BODY_LIMIT, CAT_SEGMENTS_BODY_LIMIT,
    CAT_SHARDS_BODY_LIMIT, EXHAUSTIVE_JOB_BODY_LIMIT, HEALTH_BODY_LIMIT, METRICS_BODY_LIMIT,
...
pub(crate) async fn run(cli: Cli, auth_config: Option<AuthConfig>) {
    ...
    let app = Router::new()
        ...
        .route("/_vocab/aliases/discover", post(cluster_discover_aliases))
        .route(
            "/_vocab/aliases/discover_and_record",
            post(cluster_discover_and_record_aliases),
        )
        .route("/_vocab/aliases/feedback", get(cluster_get_alias_feedback))
        .route(
