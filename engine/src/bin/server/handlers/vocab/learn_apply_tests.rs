use super::learn_apply::{
    learn_and_apply_vocab, vocab_learn_apply_method_not_allowed, VOCAB_LEARN_APPLY_BODY_LIMIT,
};
...
#[tokio::test]
async fn own_corpus_learning_applies_synchronously_and_is_observable() {
    let state = memory_state();
    let (status, headers, bytes) = send(
        &state,
        Method::POST,
        "/_vocab/learn_and_apply?min_count=2",
        Body::empty(),
    ...
    assert!(
        ...
        "the coherent learned vocabulary must be published"
    );
    assert_eq!(
        state
            .prom
    ...
    assert_eq!(
        state
            ...
            .with_label_values(&["vocab_learn_apply"])
            .get_sample_count(),
        1
    );
}
...
#[tokio::test]
async fn transport_rejects_method_unknown_query_body_size_and_stall() {
    let state = memory_state();
    ...
    let (status, headers, bytes) = send(
        &state,
        Method::POST,
        "/_vocab/learn_and_apply?refresh=true",
        Body::empty(),
        VOCAB_LEARN_APPLY_BODY_LIMIT,
    )
    .await;
...
#[tokio::test]
async fn learning_controls_are_strict_and_bounded() {
    let state = memory_state();
    for uri in [
        "/_vocab/learn_and_apply?min_count=0",
        "/_vocab/learn_and_apply?npmi_tau=0.5",
        "/_vocab/learn_and_apply?corpus_phrases=true&npmi_tau=1.1",
        "/_vocab/learn_and_apply?corpus_phrases=true&npmi_tau=NaN",
        "/_vocab/learn_and_apply?corpus_phrases=true&npmi_min_count=0",
...
async fn admission_and_engine_lock_waits_do_not_block_tokio() {
    ...
    assert!(
        tokio::time::timeout(Duration::from_millis(50), &mut request)
            .await
            .is_err(),
        "learn-and-apply must wait asynchronously for administrative admission"
    );
    drop(held);
