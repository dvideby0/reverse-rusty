...
| `src/cluster/ring.rs`, `allocator.rs` | **Placement primitives.** `HashRing` = the consistent-hash ring over anchor `FeatureId` (entity-anchor sharding ⇒ ~2–5 shard fan-out, never all N). `allocator` = the rendezvous (HRW) shard→node planner (balanced, deterministic, ≈1/N minimal-movement; drives `register_node`/`rebalance`). | [clustering-and-scaling.md](docs/design/clustering-and-scaling.md) §3; ADR-042 |
