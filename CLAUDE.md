...
| `tests/crash_injection.rs` | **Real-process SIGKILL crash-injection** (Phase 0 item 3, ADR-088): spawns the `crashwriter` bin, delivers a real external SIGKILL mid durable-op (WAL append / flush / compaction / backup / churn), reopens in-process, and diffs the recovered engine against the ADR-087 independent oracle — zero FN on every ACKed write, no resurrection/corruption (the FP reference excludes durably-deleted ids; churn prepends self-matching **canary** deletes). `#[ignore]`d behind the `check.sh` crash lane (`RR_CRASH_ITERS`); mutation-validated 3/3 (FN/FP/kill). Closes the real-kill-mid-syscall gap the chmod/torn-tail/CRC simulations cannot reach. | [testing.md](docs/testing.md); ADR-088 |
