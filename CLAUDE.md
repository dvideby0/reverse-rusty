...
| `src/bin/controlserver.rs` | Deployable cluster-manager node: serves the openraft `ControlService` over gRPC (feature `distributed`); `--bootstrap` forms the initial cluster from `--peer ID=URL` members | ADR-038 |
