...
| `src/storage.rs` + `storage/` | Persistence + on-disk formats (module). Root holds the shared binary primitives (CRC-32, atomic rename, LE scalar read/write) + public re-exports; submodules split by concern: `segment` (the `.seg` format — `write_segment` + the mmap-backed `MmapSegment` read view, incl. `class_counts` + frozen hash tables), `dict` (feature-dict (de)serialization), `manifest` (engine `Manifest` + coordinator `ClusterManifest` **v3** — per-shard segment registry + `next_seg_id`s + dict + serialized vocab, the cluster's atomic commit point), `sources` (`SourceStore` query-source persistence, `sources.dat`) | ADR-012, ADR-014, ADR-031, ADR-032, ADR-046 |
