...
| `src/wal.rs` | Write-ahead log: append-only CRC-framed entries (**v3**: deletes log one address-free `DeleteByLogical` frame, replayed via the live path's funnel — compaction-renumber-immune, ADR-066; **v4**: the `Upsert` frame — atomic replace-by-id, both halves recover or neither, ADR-067), crash recovery replay | ADR-013, ADR-066, ADR-067 |
