...
| `/_cluster/rebalance` | POST | Strict native whole-cluster HRW rebalance. Empty body is topology-safe: advisory map commit in-process, data-moving move-then-commit on assignment-routed remote coordinators, and fail-closed rejection under static remote routing. Remote `move:false` is rejected. Positive `max_parallel` enables conflict-free move waves. Returns final control `version` plus complete/resumable outcomes (ADR-042/090/095/166; [cluster control](api/cluster.md)) |
