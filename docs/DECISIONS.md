...
| [060](decisions/adr-060-bulk-synonym-alias-registration.md) | Bulk synonym / alias registration (Solr-format file loader) | Parse the Solr/Lucene synonym-file format (`a, b, c` sets + `a, b => c` mappings, `#` comments) and map every rule onto FN-safe equivalence **expansion** (ADR-054) — `=>` sides unioned (direction immaterial to recall); multi-token forms glued as phrases. Surfaces: `vocab::parse_synonyms` + `Vocab::extend_from_synonyms[_file]` + bulk `add_equivalences`/`add_synonyms` (lean core) + `POST /_vocab/synonyms` (raw body, merge + recompile, fail-loud with line number). No directional collapse (forbidden-term footgun). Closes the last Tier-4 item. Built + oracle-proven. | Accepted |
