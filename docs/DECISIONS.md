...
| [060](decisions/adr-060-bulk-synonym-alias-registration.md) | Bulk synonym / alias registration (Solr-format file loader) | Parse the Solr/Lucene synonym-file format (`a, b, c` sets + `a, b => c` mappings, `#` comments) and map every rule onto FN-safe equivalence **expansion** (ADR-054) — `=>` sides unioned (direction immaterial to recall); multi-token forms glued as phrases (alias entities, ADR-061). Surfaces: `vocab::parse_synonyms` + `Vocab::extend_from_synonyms[_file]` + bulk `add_equivalences`/`add_synonyms` (lean core) + `POST /_vocab/synonyms` (raw body, merge + recompile, fail-loud with line number). No directional collapse (forbidden-term footgun). Closes the last Tier-4 item. Built + oracle-proven. | Accepted |
| [061](decisions/adr-061-alias-entity-phrases.md) | Alias-entity phrases (ES `synonym_graph` equivalent) | Make multi-word aliases genuinely bidirectional like ES `synonym_graph`: an **alias-entity phrase** collapses on the query side (emit only the entity feature → equivalence-expanded to synonyms) but stays additive on the title side (entity + components, so component queries still match). Plumbed via a `query_side` flag through `emit`; `resolve_equivalences` collapses the raw multi-word member to one entity. Safe-direction asymmetry (titles emit a superset ⇒ lossless cover holds); the one sanctioned exception to "same normalizer". A multi-word-form query becomes phrasal (ES-equivalent). Default path byte-identical. Built + oracle-proven (bidirectional + phrasal-adjacency + engine ≡ brute). | Accepted |
