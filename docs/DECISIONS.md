...
| 047 | Remote live-write partial-apply — observe, fail-closed, repair (`resync`) + the `block_on` thread-context contract | Accepted |
| 048 | Reliability hardening — auto-unfence-on-abort, translog-lease TTL, autoscaler-driven handoff | Accepted |
| 049 | Per-query metadata, filtered percolation, and optional ranking (percolator parity) | Accepted |
