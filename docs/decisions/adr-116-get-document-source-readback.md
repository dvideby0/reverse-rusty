...
- **REST contract.** Found documents return `_index: "queries"`, numeric `_id`, the stored
  ...
  bodyless 200/404 existence check. Single-node and in-process coordinator modes are identical;
...
- **Compatibility.** v1 files migrate to extended v2 and original v2 files read unchanged. Their
  query text remains available, their write version is corrected from the newest live exact row,
  and dense legacy tags are reconstructed through the persisted `TagDict`. A pre-footer synthetic
  tag has no reversible string; that rare response is explicitly marked
  ...
  as a 404. The v2 CRC and atomic rename discipline are unchanged.
- **Safety and cost.** Source metadata remains outside candidate gating, exact verification, and
  the title hot path, so the lossless-cover contract is untouched. Metadata is decoded only for a
  document point-read or a rebuild gather. Query-only search enrichment still clones exactly the
  bounded query bytes it did before.
