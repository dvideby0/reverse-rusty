...
- **Decision:** Add fallible `try_bulk_ingest` / `try_build_from_queries` returning
  ...
  deleted, so nothing is committed. The original implementation collected accepted source text
  locally and persisted `sources.dat` *after* that commit point; ADR-121 later closed that remaining
  half-commit window by making the source snapshot a manifest-selected artifact too. The server maps
  a bulk persistence failure to HTTP 503 (`/_bulk`) and aborts startup on initial-load failure rather
  than serving a non-durable engine.
- **Consequence:** A bulk batch is now durable-or-rejected: callers never get a success report for
  data that isn't on disk. Match data and its canonical source snapshot are jointly selected by
  manifest v7 (ADR-121); source failure rejects the batch. Auxiliary in-memory state interned during
  a rejected batch (new dict features, rejection counters) is not rolled back — like RocksDB,
  internal stats reflect the attempt; only the committed data set is transactional. Cost: each bulk
  call still rewrites the source store
...
- **See also:** ADR-013 (WAL — the live-insert path this deliberately contrasts with), ADR-012
  (segment file format + manifest), ADR-014 (query source store), ADR-121 (joint source commit),
  P2-13 (`durable_rename`),
  [ingestion-and-updates.md](../design/ingestion-and-updates.md)
- **Amended by [ADR-121](adr-121-atomic-source-sidecar-commit.md):** source publication is no
