...
- **Decision.** Parse the **Solr/Lucene synonym-file format** — the de-facto interchange format that
  ...
  3. **Multi-token forms are glued.** A form like `upper deck` / `i-pod` can't be a single feature on its
     own, so it is registered as a collapse **phrase** (`["upper","deck"] -> "term:upper_deck"`) and the
     glued feature joins the equivalence group. (It tightens to adjacency — the documented
     [ADR-053](adr-053-corpus-phrase-vocab-source.md) residual — but that is the operator's explicit
     choice to declare the form a unit, and the same normalizer runs over titles, so the cover holds.)
