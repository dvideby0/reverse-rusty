...
- **Context:** Some queries are inherently non-selective (for example a bare `wireless` with no
  model, brand, or other discriminator). In a flat index these poison the hot path — one broad posting list can
  dominate match time.
