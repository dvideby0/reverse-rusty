...
- **Context.** `Engine::tombstone_in(segment, local_id)` is an expert physical-address API.
  ...
  unsafe because it replaces and renumbers segments; callers holding old addresses must be told to
  re-resolve them.
...
- **Decision.** Validate the segment, local-id bounds, and liveness before touching the WAL.
  Rejections return the public typed `TombstoneError` (`SegmentNotFound`, `LocalNotFound`,
  `AlreadyDeleted`, or the defensive `SegmentIndexOverflow`) and consume no WAL sequence or bytes.
  Once the address is validated, preserve ADR-013's WAL-first order; `TombstoneError::Wal` leaves
  the row alive. WAL recovery remains deliberately idempotent and tolerant of invalid/already-dead
  historical frames because replay is not a new caller acknowledgement.
...
- **Testing.** Unit coverage pins invalid segment, invalid local id, already-dead row, zero WAL
  ...
  WAL-tail replay and a held address invalidated by compaction, while ADR-066's existing
