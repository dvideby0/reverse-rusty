...
| **Control-plane restart** | Each node resumes from its durable Raft log/vote (ADR-041). | Restart control nodes; quorum re-forms. With control wiring on (compose/Helm default), the coordinator's thin client fails over across endpoints meanwhile — reads stay available; admin writes fail loud until quorum returns (ADR-086). |
