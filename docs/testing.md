...
| Persistence | `tests/persistence.rs` + storage/cluster unit modules | Segment round-trip, WAL crash-recovery replay, mmap compaction, durability-failure events, ADR-108 typed-priority v6/legacy fallback, and ADR-109 segment-v7 / cluster-manifest-v6 / clog+translog-v4 / adopted-space-v2 round-trips, malformed-column refusal, and rebuild-only migration fences. ADR-110 adds top-K/source continuity through checkpoint/reopen and backup/restore with **no new format version**. Standalone segments v1–v6 remain readable. |
