...
| **Independent oracle** | `tests/independent_oracle/` | **Front-end code-independent differential** (Phase 0 item 2, ADR-087): the engine diffed against `reverse-rusty-ref-matcher` — a std-only, zero-dependency reimplementation of the parser/normalizer/extractor/predicate from the spec that shares NO front-end code (independence enforced by a `check.sh` `cargo tree` lane). Zero FN/FP over generated default (clean + messy), populated graders/phrases, the multi-word alias two-view (controlled + ~989k-match at-scale), a hand-written **gotcha** table (asserted against both sides), and the env-gated `RR_ORACLE_CORPUS` real corpus (schema below). ADR-118 proved the boundary: both implementations can still copy one ambiguous semantic rule; its clause-boundary case is now a human-expectation pin, and issue #123 tracks a stronger semantic-model reference. |
...
| Persistence | `tests/persistence.rs` + storage/cluster unit modules | Segment round-trip, WAL crash-recovery replay, mmap compaction, durability-failure events, ADR-108 typed-priority v6/legacy fallback, and ADR-109 segment-v7 / cluster-manifest-v6 / clog+translog-v4 / adopted-space-v2 round-trips, malformed-column refusal, and rebuild-only migration fences. ADR-118 adds compiler-semantics 0/1 codec coverage; cluster-manifest-v7 and shard-checkpoint-v2 round trips; one-shot standalone migration with aliases and with number context only; newly exposed dense-ID stability; equivalence-aware legacy `open` + `adopt_vocab`; duplicate-row/degraded-subset refusal; WAL-tail source survival across a second reopen; coherent live state after source persistence failure; above-default accepted-limit recovery; RF=2 local-cluster rebuild/re-placement; legacy-tail logical folding before current placement validation; empty-base coordinator/translog-tail fences; failed-cluster-migration source retry safety; fail-loud future semantics/shard-local restart without advancing its sidecar/translog commit point; and mixed-peer handshake refusal before adoption. ADR-110 adds top-K/source continuity through checkpoint/reopen and backup/restore with **no new layout version**. Standalone segment layouts v1–v8 remain readable subject to their semantic migration/fence checks. |
...
is the differential complement they structurally cannot be. ADR-118 found the residual boundary:
independent code can still implement the same ambiguous prose incorrectly, so human semantic pins
remain load-bearing; issue #123 tracks making that semantic layer more independent (full rationale →
[`DECISIONS.md`](DECISIONS.md) ADR-087/118).
